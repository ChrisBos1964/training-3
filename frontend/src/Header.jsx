...
function Header() {
  ...
  useEffect(() => {
    ...
    setTheme(savedTheme);
    // Theme is already applied by the script in HTML head, so we just need to sync state
  }, []);
  ...
  const applyTheme = (newTheme) => {
    ...
    if (newTheme === 'auto') {
      // Remove any existing theme classes and let CSS handle auto
        ...
        <div className="header-content">
          ...
          <ul className="nav-menu" aria-label="Main Menu" data-testid="main-menu">
            ...
            <li>
              <a
                href="/shadow-dom"
                aria-label="View Shadow DOM demo page"
                aria-current={location.pathname === '/shadow-dom' ? 'page' : undefined}
                onClick={(e) => { e.preventDefault(); handleShadowDomClick(); }}
                className={`nav-button ${location.pathname === '/shadow-dom' ? 'active' : ''}`}
                role="link"
                data-testid="nav-shadow-dom"
              >
                Shadow DOM
              </a>
            </li>
            <li className="login-container">
              {isLoggedIn ? (
                <>
                  <a
                    href="/login"
                    aria-label="You are logged in - click to logout"
                    aria-current={location.pathname === '/login' ? 'page' : undefined}
                    onClick={(e) => { e.preventDefault(); handleLoginClick(); }}
                    onMouseEnter={() => setIsHoveringLogin(true)}
                    onMouseLeave={() => setIsHoveringLogin(false)}
                    className={`nav-button logged-in-status ${location.pathname === '/login' ? 'active' : ''}`}
                    role="link"
                    data-testid="nav-login"
                    title={userInfo?.email ? `Logged in as ${userInfo.email} via Google` : 'Logged in'}
                  >
                    {isHoveringLogin ? 'Log out' : 'Logged In'}
                  </a>
                  {userInfo?.avatar_url && (
                    <img 
          ...
          >
            {theme === 'light' && (
              <svg className="w-full light-icon" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path fillRule="evenodd" clipRule="evenodd" d="M16.0004 24.9146C15.4644 24.9146 15.0304 25.3496 15.0304 25.8846V28.6536C15.0304 29.1886 15.4644 29.6236 16.0004 29.6236C16.5364 29.6236 16.9704 29.1886 16.9704 28.6536V25.8846C16.9704 25.3496 16.5364 24.9146 16.0004 24.9146" fill="currentColor"></path>
                <path fillRule="evenodd" clipRule="evenodd" d="M28.6542 15.0298H25.8842C25.3492 15.0298 24.9152 15.4648 24.9152 15.9998C24.9152 16.5348 25.3492 16.9688 25.8842 16.9688H28.6542C29.1902 16.9688 29.6242 16.5348 29.6242 15.9998C29.6242 15.4648 29.1902 15.0298 28.6542 15.0298" fill="currentColor"></path>
                <path fillRule="evenodd" clipRule="evenodd" d="M22.9896 9.97995C23.2376 9.97995 23.4856 9.88495 23.6756 9.69595L24.7036 8.66795C25.0816 8.28995 25.0816 7.67495 24.7036 7.29595C24.3246 6.91795 23.7106 6.91795 23.3316 7.29595L22.3036 8.32495C21.9256 8.70295 21.9256 9.31695 22.3036 9.69595C22.4926 9.88495 22.7416 9.97995 22.9896 9.97995" fill="currentColor"></path>
                <path fillRule="evenodd" clipRule="evenodd" d="M8.32507 9.69593C8.51407 9.88493 8.76207 9.97993 9.01107 9.97993C9.25907 9.97993 9.50707 9.88493 9.69607 9.69593C10.0751 9.31693 10.0751 8.70293 9.69607 8.32493L8.66807 7.29693C8.28907 6.91893 7.67507 6.91893 7.29707 7.29693C6.91807 7.67493 6.91807 8.28993 7.29707 8.66793L8.32507 9.69593Z" fill="currentColor"></path>
                <path fillRule="evenodd" clipRule="evenodd" d="M8.32507 22.3043L7.29707 23.3313C6.91807 23.7093 6.91807 24.3243 7.29707 24.7023C7.48607 24.8923 7.73407 24.9873 7.98207 24.9873C8.23007 24.9873 8.47807 24.8923 8.66807 24.7023L9.69607 23.6753C10.0751 23.2973 10.0751 22.6833 9.69607 22.3043C9.31807 21.9253 8.70307 21.9253 8.32507 22.3043" fill="currentColor"></path>
                <path fillRule="evenodd" clipRule="evenodd" d="M23.6752 22.3043C23.2962 21.9253 22.6822 21.9253 22.3032 22.3043C21.9252 22.6833 21.9252 23.2973 22.3042 23.6753L23.3322 24.7023C23.5212 24.8923 23.7692 24.9873 24.0182 24.9873C24.2662 24.9873 24.5142 24.8923 24.7032 24.7023C25.0822 24.3243 25.0822 23.7093 24.7032 23.3313L23.6752 22.3043Z" fill="currentColor"></path>
              </svg>
            )}
            {theme === 'dark' && (
            ...
            {theme === 'auto' && (
              <svg width="24" height="24" fill="none" viewBox="0 0 24 24" className="auto-icon">
                <path d="M5.75 5.75a1 1 0 011-1h10.5a1 1 0 011 1v8.5H5.75v-8.5zM18.25 14.5l.746 3.544a1 1 0 01-.979 1.206H5.982a1 1 0 01-.978-1.206L5.75 14.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
              </svg>
            )}
          </button>
        </div>
